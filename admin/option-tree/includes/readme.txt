...
Tags: options, Opções do Tema, meta boxes
Requires at least: 3.8
Tested up to: 4.2
Stable tag: 2.5.5
License: GPLv3

Opções do Tema UI Builder for WordPress. A simple way to create & save Opções do Tema and Meta Boxes for free or premium themes.
...
Build your Opções do Tema panel locally with an easy to use drag & drop interface and then export a functioning `theme-options.php` file for production use that is i18n translation ready, with your custom text domain automatically inserted. 

And, in just a few simple lines of code, save settings to the database with a unique array ID so none of your Opções do Tema conflict with other themes that use OptionTree. 
...
* Hotfix - Added a settings ID auto-fill that is based on the text of the settings label in the Opções do Tema UI Builder. Contributors via github @valendesigns and @Ore4444.
...
= 2.4.1 =
* Hotfix - Fixed a typo in the demo Opções do Tema related to the `social-links`.
...
* Added an admin notice when the UI Builder is being overridden by custom Opções do Tema.
...
* Added filter 'ot_theme_options_contextual_help' which allows you to filter the Contextual Help on the Opções do Tema page.
* Added filter 'ot_theme_options_sections' which allows you to filter the Sections on the Opções do Tema page. Contributors via github @joshlevinson.
* Added filter 'ot_theme_options_settings' which allows you to filter the Settings on the Opções do Tema page. Contributors via github @joshlevinson.
...
* Created functions to register the Opções do Tema & Settings pages, and with better filtering.
...
* Added filter 'ot_show_options_ui' which allows you to hide the Opções do Tema UI Builder.
...
* Allow "New Layout" section to be hidden on the Opções do Tema page via a filter.
...
* Better Opções do Tema UI Builder.
...
* Can now save layouts from the Opções do Tema page.
...
* Allow layout change on Opções do Tema page.
...
* Only load option type function on Opções do Tema page
...
= 1.1.6 =
Added theme integration for developers. It's now possible to have a default XML file included in your theme to populate the Opções do Tema and hide the settings and docs pages. Read more about this in the plugins built in documentation.
